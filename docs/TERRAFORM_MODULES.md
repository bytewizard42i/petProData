...
Provisions, for petProData: a per-app CockroachDB database, a least-privilege
login role, table grants, and (optionally) the connection string in AWS Secrets
Manager. Pet records are objects (rwa_objects) with attached credentials.
...
module "helixchain" {
  ...
  app_name     = "petprodata"
  db_password  = var.petprodata_db_password   # TF_VAR_petprodata_db_password
  cluster_host = var.cluster_host
  grants = {
    rwa_objects = ["SELECT", "INSERT"]   # pet records as objects
    credentials = ["SELECT", "INSERT"]   # vet/vaccination VCs
  }
}
```
